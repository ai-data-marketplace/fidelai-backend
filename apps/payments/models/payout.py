...
class WithdrawalRequest(TimeStampedModel):
	...
	payment_method = models.CharField(max_length=30, choices=WithdrawalMethodChoices.choices)
	...
	class Meta:
		...
		constraints = [
			...
			models.CheckConstraint(
				condition=(
					models.Q(status=WithdrawalStatusChoices.COMPLETED, processed_at__isnull=False)
...
class CommissionConfig(TimeStampedModel):
	role = models.CharField(max_length=20, choices=CommissionRoleChoices.choices)
	...
	class Meta:
		...
		constraints = [
			models.CheckConstraint(
				condition=models.Q(commission_percentage__gte=0) & models.Q(commission_percentage__lte=100),
...
class PayoutRule(TimeStampedModel):
	...
	minimum_withdrawal_amount = models.DecimalField(max_digits=14, decimal_places=2)
	score_to_currency_rate = models.DecimalField(max_digits=14, decimal_places=6)
	active = models.BooleanField(default=True)
